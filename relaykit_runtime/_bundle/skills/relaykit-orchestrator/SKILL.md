...
description: Use when a RelayKit lane owns routing, sequencing, checkpointing, or convergence for a task. Load this with a host guide and model note so orchestration can move between Codex, Claude Code, Antigravity, or Gemini CLI without changing lane semantics.
license: MIT
compatibility:
  hosts:
    - Codex
